## 📚 Companion Resources for *Building a Career in Software Engineering*

This folder contains the **same curated resources referenced at the end of each chapter** in the book  
**_Building a Career in Software Engineering_** by Ifeanyi Nnamdi-Okagbue.

At the end of **every chapter in the book**, readers are provided with carefully selected resources to help them:

- Reinforce the key concepts covered in that chapter  
- Go deeper into specific topics, tools, or skills  
- Apply what they’ve learned through practice and exploration  

This repository brings those chapter-by-chapter resources together in one place for easy reference.

---

### ⚠️ Important

These resources are **supplements**, not substitutes.

They are designed to **consolidate and extend what you’ve already read in the book**.  
The structure, context, explanations, and real-world guidance live in the chapters themselves.

If you jump straight into the resources without reading the chapter, you’ll miss the “why” behind them.

---

## Chapter 1: Laying the Foundation – Trainee to Junior Engineer

### Chapter focus

This chapter focuses on building a strong foundation for a career in software engineering. It covers:

- Core programming fundamentals  
- Problem-solving and debugging skills  
- Version control and collaborative development  
- Essential computer science concepts  

The resources below are the **exact materials referenced at the end of Chapter 1**, curated to help you strengthen these fundamentals and put them into practice.

### 🧠 Foundational Programming Skills

**Books**
- *The C Programming Language* — Brian W. Kernighan & Dennis M. Ritchie  
  https://www.amazon.com/Programming-Language-2nd-Brian-Kernighan/dp/0131103628
- *You Don’t Know JS* — Kyle Simpson  
  https://www.amazon.com/You-Dont-Know-JS-Yet/dp/B084DFZ6GW

**Online Courses**
- CS50’s Introduction to Computer Science (Harvard, edX)  
  https://www.edx.org/learn/computer-science/harvard-university-cs50-s-introduction-to-computer-science
- Python for Everybody (Coursera)  
  https://coursera.org/specializations/python

**Practice Platforms**
- Codecademy – Interactive programming exercises  
  https://www.codecademy.com/
- freeCodeCamp – Web development, algorithms, and projects  
  https://www.freecodecamp.org/

---

### 🐞 Debugging & Problem-Solving

**Books**
- *Debugging: The 9 Indispensable Rules* — David J. Agans  
  https://www.amazon.com/Debugging-Indispensable-Software-Hardware-Problems/dp/0814474578
- *Programming Pearls* — Jon Bentley  
  https://www.amazon.com/Programming-Pearls-2nd-Jon-Bentley/dp/0201657880

**Articles**
- *Debugging Like a Developer* – Rithm School  
  https://medium.com/@rithmschool/debugging-like-a-developer-cc26b0bf7444
- *10 Common Coding Mistakes and How to Avoid Them*  
  https://medium.com/@miismail007/10-common-coding-mistakes-and-how-to-avoid-them-0710b68f93d8

**Tools**
- Visual Studio Code & built-in debugger  
  https://code.visualstudio.com/
- Postman – Debugging and testing APIs  
  https://www.postman.com/

**Practical Action**
- Intentionally debug a broken program instead of rewriting it
- Practice explaining *why* a bug happened, not just how you fixed it

---

### 🌱 Version Control (Git & GitHub)

**Books**
- *Pro Git* — Scott Chacon & Ben Straub (free online)  
  https://www.amazon.com/Pro-Git-Scott-Chacon-ebook/dp/B01ISNIKES

**Courses & Tutorials**
- Git Essential Training – LinkedIn Learning  
  https://www.linkedin.com/learning/git-essential-training-19417064
- GitHub Learning Lab  
- Learn Git Branching (interactive)  
  https://learngitbranching.js.org/

**Practical Action**
- Create a GitHub repository for every learning project
- Practice branching, rebasing, and writing meaningful commit messages

---

### 🤝 Collaborative Development & Teamwork

**Books**
- *Team Geek* — Ben Collins-Sussman, Brian W. Fitzpatrick, Michael W. Pilato  
  https://www.amazon.com/Team-Geek-Software-Developers-Working/dp/1449302440

**Videos**
- Agile for Developers (YouTube)  
  https://www.youtube.com/watch?v=xp5kbbEDPOE

**Articles**
- *Effective Code Review Practices* – Hatica Blog  
  https://www.hatica.io/blog/code-review-practices/

**Practical Action**
- Participate in code reviews (even for open-source projects)
- Learn to give feedback that is clear, kind, and actionable

---

### 🧩 Core Computer Science Foundations

**Books**
- *Introduction to Algorithms* — Thomas H. Cormen et al.  
  https://www.amazon.com/Introduction-Algorithms-fourth-Thomas-Cormen/dp/026204630X
- *Computer Networking: A Top-Down Approach* — Kurose & Ross  
  https://www.amazon.com/Computer-Networking-Top-Down-Approach-6Th/dp/9332585490

**Courses**
- Algorithms (Princeton University, Coursera)  
  https://www.coursera.org/learn/algorithms-part1
- Computer Science 101 (Udemy)  
  https://www.udemy.com/course/computer-science-101-computers-programming-for-beginners

**Platforms**
- Khan Academy – Computer science fundamentals  
  https://www.khanacademy.org/
- Educative – Interactive CS and algorithm courses  
  https://www.educative.io/

---

## 📌 How to Use These Resources

- Do **not** try to consume everything at once
- Use resources **only when the book recommends them**
- Focus on understanding concepts, not finishing courses
- Apply what you learn through small, consistent projects

---

## 🙌 Contributing & Feedback

If you have suggestions for better resources or updates:
- Open an issue
- Submit a pull request
- Share feedback from your learning experience

---

📘 **Want the full context, stories, and guidance?**  
Get the book: [*Building a Career in Software Engineering*](https://okagbue.gumroad.com/l/software-engineer-career-guide)
